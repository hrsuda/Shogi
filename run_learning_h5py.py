...
def main():
    ...
    test_bool = data_t.astype(bool)
    ind = np.arange(test_bool.shape[0],dtype=int)
    ind_good_test = ind[test_bool][np.random.choice(len(ind[test_bool]),int(test_len/2))]
    ind_bad_test = ind[~test_bool][np.random.choice(len(ind[~test_bool]),int(test_len/2))]

    test_ind = np.concatenate([ind_good_test,ind_bad_test])
    ...
    ind_good = np.arange(len(t_data_good))
    ind_bad = np.arange(len(t_data_bad))
    ...
    for i in range(iters_num):
        ...
        if i % 500 == 0:
            ...
            learning_rate = learning_rate * 9.95e-1
