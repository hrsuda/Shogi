...
class Tanh(object):
    ...
    def forward(self, x):
        # x = x - cp.max(x, axis=-1, keepdims=True) + 1e-12
        out = cp.tanh(x)
        self.out = out
        return out
...
class Convolution:
    ...
    def backward(self, dout):
        ...
        self.db = cp.sum(dout.reshape(-1,9,9),axis=0)
...
class Flatten(object):
    ...
    def _backward2(self, dout):
        ...
        for i in range(self.data_num):
            shape = self.shapes[i]
            stop = start + cp.prod(cp.array(shape[1:]))
...
class Duplication(object):
    ...
    def backward(self, dout):
        out = cp.sum(cp.array(dout[self.ind:self.ind+self.num]), axis=0)/self.num
...
class Combine(object):
    ...
    def forward(self, x):
        ...
        c = cp.array(c)
        c = cp.concatenate(c, axis=1)
        # ax_num = cp.arange(len(c.shape))
    ...
    def backward(self, dout):
        c = dout[0]
        c = cp.split(c,len(self.ind),axis=1)
        d = dout[1:]
